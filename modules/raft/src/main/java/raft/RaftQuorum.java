...
//        electionTimeOutCheckTask = timeWheel.addTask(new ElectionTimeOutCheckTask(), false);
        ...
        stateMachine
                .addTransition(RaftState.FOLLOWER, RaftEvent.HEARTBEAT_TIMEOUT, RaftState.CANDIDATE, new Transition() {
                    ...
                    public boolean doTransition() {
                        ...
                        resetTimeOut();
                        LOG.info(instance.getDescription() + "\tFOLLOWER -> CANDIDATE (WITH HEARTBEAT_TIMEOUT) TERM " + term);
                        return true;
                    }
                })
                .addTransition(RaftState.CANDIDATE, RaftEvent.CANDIDATE_ELECTION_TIMEOUT, RaftState.CANDIDATE, new Transition() {
                    @Override
                    public boolean doTransition() {
                        setLeader(null);
                        resetTimeOut();
                        ...
                        if (stateMachine.getCurrentState() != RaftState.CANDIDATE) {
                            ...
                            LOG.info(instance.getDescription() + "\tCANDIDATE -> CANDIDATE (WITH CANDIDATE_ELECTION_TIMEOUT) TERM " + term);
                            return true;
                        }
                    }
                })
                .addTransition(RaftState.CANDIDATE, RaftEvent.RECEIVE_VOTES_FROM_MAJORITY_SERVERS, RaftState.LEADER, new Transition() {
                    @Override
                    public boolean doTransition() {
                        timeWheel.removeTask(heartBeatCheckTask);
                        timeWheel.removeTask(electionTimeOutCheckTask);
                        ...
                        LOG.info(instance.getDescription() + "\tCANDIDATE -> LEADER (WITH RECEIVE_VOTES_FROM_MAJORITY_SERVERS) TERM " + term);
                        return true;
                    }
                })
                .addTransition(RaftState.CANDIDATE, RaftEvent.DISCOVERY_CURRENT_LEADER, RaftState.FOLLOWER, new Transition() {
                    @Override
                    public boolean doTransition() {
                        resetTimeOut();
                        reset();
                        LOG.info(instance.getDescription() + "\tCANDIDATE -> FOLLOWER (WITH DISCOVERY_CURRENT_LEADER) TERM " + term);
                        return true;
                    }
                })
                .addTransition(RaftState.CANDIDATE, RaftEvent.DISCOVERY_HIGH_TERM, RaftState.FOLLOWER, new Transition() {
                    @Override
                    public boolean doTransition() {
                        resetTimeOut();
                        reset();
                        LOG.info(instance.getDescription() + "\tCANDIDATE -> FOLLOWER (WITH DISCOVERY_HIGH_TERM) TERM " + term);
                        return true;
                    }
                })
                .addTransition(RaftState.LEADER, RaftEvent.DISCOVERY_HIGH_TERM, RaftState.FOLLOWER, new Transition() {
                    ...
                    public boolean doTransition() {
                        ...
                        reset();
                        resetTimeOut();
                        LOG.info(instance.getDescription() + "\tLEADER -> FOLLOWER (WITH DISCOVERY_HIGH_TERM) TERM " + term);
                        return true;
                    }
                });
    }
    ...
    public void vote(com.lightgraph.raft.proto.VoteRequest request,
        ...
        ConsensusInstance candidate = new ConsensusInstance(request.getCandidate().toByteArray());
        ...
        synchronized (stateMachine) {
            try {
                ...
                } else if (requestTerm > this.term && requestIndex < currentIndex && stateMachine.getCurrentState() == RaftState.LEADER) {
    ...
    public void keepHeartBeat(com.lightgraph.raft.proto.HeartBeatRequest request,
        ...
        long requestTerm = request.getTerm();
        ConsensusInstance requestLeader = new ConsensusInstance(request.getLeader().toByteArray());
        synchronized (stateMachine) {
            try {
    ...
    public void sendVoteRequest() {
        ...
        builder.setCandidate(ByteString.copyFrom(instance.getBytes()));
        ...
        if (instance.getGroupSize() == 1) {
            ...
            for (ConsensusInstance instance : instances) {
                ...
                builder.setInstance(ByteString.copyFrom(instance.getBytes()));
                    ...
                    public void onNext(VoteResponse voteResponse) {
                        ...
                        if (accept) {
                            synchronized (stateMachine) {
                                ...
                                if (curVotes > instance.getGroupSize() / 2 && stateMachine.getCurrentState() != RaftState.LEADER) {
    ...
    public class HeartBeatBoardCastTask implements Task {
        ...
        public Object doTask() {
            ...
            builder.setTerm(term);
            builder.setLeader(ByteString.copyFrom(instance.getBytes()));
            builder.setIndex(logSystem.getLatestEditIndex());
